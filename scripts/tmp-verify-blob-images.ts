 ...
 * Run: pnpm exec tsx scripts/tmp-verify-blob-images.ts
 */
...
import { externalizeAppImages } from "../actions/_blob-images.js";

console.log("providers:", listFileUploadProviders().map((p) => `${p.id} (${p.isConfigured() ? "configured" : "not"})`).join(", "));

const app = {
  images: [
    { id: "i1", name: "tiny png", image: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==", imageIsURL: false },
