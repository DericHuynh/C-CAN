...
/** Edit the viewer's CSS without applying authored rules to the editing tools. */
export function CustomCssPanel({ project }: CustomCssPanelProps) {
  const app = project.app;
  const updateSettings = useUpdateProjectSettings();

  const appliedCss = app.customCSS ?? "";
  const [text, setText] = useSavedField(appliedCss, ["customCSS"]);
  ...
  function handleClear() {
    const clearedDraft = text;
    updateSettings.mutate(
      { projectId: project.id, patch: { customCSS: "" } },
      {
        onSuccess: () => {
          setText((current) => (current === clearedDraft ? "" : current));
