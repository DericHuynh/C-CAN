...
export function CategoriesPanel({ project }: CategoriesPanelProps) {
  ...
  const [selected, setSelected] = useLiveSelection<Category>(categories, (item) =>
    JSON.stringify([item.type, item.idx]),
  );
  ...
  function handleDelete() {
    ...
    updateSettings.mutate(
      ...
      {
        onSuccess: () => {
          if (selected !== "new" && selected?.idx === target.idx && selected?.type === target.type)
            setSelected(null);
...
function CategoryForm({
  ...
  const [name, setName] = useSavedField(
    category?.name ?? "",
    category ? ["$categories", category.type, String(category.idx), "name"] : null,
  );
  const [type, setType] = useSavedField(
    category?.type ?? defaultType ?? typeOptions[0] ?? "",
    category ? ["$draft", "category", String(category.idx), category.type, "type"] : null,
  );
