...
function ProjectDetailsCard({ project, busy = false, onSave }: ProjectDetailsCardProps) {
  const [title, setTitle] = useSavedField(project.title ?? "", ["$metadata", "title"]);
  const [description, setDescription] = useSavedField(project.description ?? "", [
    "$metadata",
    "description",
  ]);
...
function ViewerCard({ title, busy = false, onSave }: ViewerCardProps) {
  const [value, setValue] = useSavedField(title, ["viewerConfig", "title"]);
...
function DefaultsCard({ defaults, busy = false, onSave }: DefaultsCardProps) {
  const [defaultRowTitle, setDefaultRowTitle] = useSavedField(defaults.defaultRowTitle, [
    "defaultRowTitle",
  ]);
  const [defaultChoiceTitle, setDefaultChoiceTitle] = useSavedField(defaults.defaultChoiceTitle, [
    "defaultChoiceTitle",
  ]);
  const [defaultBeforePoint, setDefaultBeforePoint] = useSavedField(defaults.defaultBeforePoint, [
    "defaultBeforePoint",
  ]);
  const [defaultAfterPoint, setDefaultAfterPoint] = useSavedField(defaults.defaultAfterPoint, [
    "defaultAfterPoint",
  ]);
