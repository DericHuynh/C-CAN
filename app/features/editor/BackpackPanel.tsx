...
export function BackpackPanel({ project }: BackpackPanelProps) {
  ...
  function commit(next: Row[], onSuccess?: () => void) {
  ...
  function addRow() {
    const next = [...sorted, { ...createDefaultRow(project.app, rows.length), isBackpack: true }];
    commit(
      next.map((row, index) => ({ ...row, index })),
  ...
  function handleSave(patch: Record<string, unknown>) {
    ...
    if (!selected) return;
    commit(
      rows.map((row) => (row.id === selected.id ? { ...row, ...patch } : row)),
  ...
  function handleDelete() {
    ...
    commit(
      sorted.filter((row) => row.id !== target.id).map((row, index) => ({ ...row, index })),
      () => {
  ...
  function move(rowId: string, delta: -1 | 1) {
    ...
    commit(next.map((row, index) => ({ ...row, index })));
  }
  ...
  return (
    <>
        ...
        master={
          <div className="space-y-3">
            <div className="sticky top-0 z-10 flex items-center justify-between gap-2 bg-background/95 py-2 backdrop-blur">
              ...
              <Button type="button" size="sm" onClick={addRow} disabled={updateSettings.isPending}>
            ...
            {rows.length === 0 ? (
              <Card>
                <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
                  ...
                  <Button type="button" onClick={addRow} disabled={updateSettings.isPending}>
                    <IconPlus className="mr-1.5 size-4" />
                    Add backpack row
                  </Button>
                </CardContent>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex shrink-0 items-center gap-0.5">
                          ...
                          <Button
                            ...
                            size="icon"
                            className="size-8 text-muted-foreground"
                            disabled={updateSettings.isPending || index === rows.length - 1}
                            onClick={(event) => {
                              event.stopPropagation();
                              move(row.id, 1);
                            }}
      ...
      <ConfirmDeleteDialog
        ...
        description="This removes the row from the build summary."
        busy={updateSettings.isPending}
