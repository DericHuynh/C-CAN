...
export function EditorPane({
  ...
  saveLabel?: string;
  canSave?: boolean;
  /** Inline canvas editing keeps focus in the text instead of the inspector. */
  focusOnOpen?: boolean;
  onCancel: () => void;
  onSave: () => void;
}) {
  ...
  useEffect(() => {
    ...
    if (isNarrow && focusOnMount.current) {
    ...
    >
      ...
      <CardFooter className="sticky bottom-0 z-10 shrink-0 flex-wrap justify-end gap-2 border-t bg-card px-6 py-4">
        <span className="mr-auto text-xs text-muted-foreground" title="Outside text fields">
          {live && saveLabel === "Save"
            ? "Live edits · Shift + S to apply all"
            : "Shift + S to save"}
        </span>
        <Button type="button" variant="outline" onClick={onCancel} disabled={busy}>
          {live && saveLabel === "Save" ? "Close" : "Cancel"}
        </Button>
        <Button
          type="button"
          onClick={onSave}
