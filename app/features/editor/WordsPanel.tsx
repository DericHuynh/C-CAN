...
export function WordsPanel({ project }: WordsPanelProps) {
  ...
  const [selected, setSelected] = useLiveSelection<Word>(words);
  ...
  function handleSave(form: WordFormValues) {
    ...
    if (
      !form.id ||
      words.some((item) => item.id === form.id && (isNew || item.id !== selected.id))
    ) {
        ...
        detail={
          selected === "new" ? (
            <WordForm
              ...
              onSave={handleSave}
            />
          ) : selected && words.some((item) => item.id === selected.id) ? (
            <WordForm
              key={selected.id}
              word={selected}
              busy={updateSettings.isPending}
...
function WordForm({ word, busy = false, onCancel, onSave }: WordFormProps) {
  const [id, setId] = useSavedField(word?.id ?? "", word ? ["words", word.id, "id"] : null);
  const [replaceText, setReplaceText] = useSavedField(
    word?.replaceText ?? "",
    word ? ["words", word.id, "replaceText"] : null,
  );
