...
import { countGroupMembers } from "@/lib/project-utils";
...
export function GroupPanel({ project }: GroupPanelProps) {
  ...
  const [selected, setSelected] = useLiveSelection<Group>(groups);
...
function GroupForm({ group, rows, busy = false, onCancel, onSave }: GroupFormProps) {
  const [value, setValue] = useSavedField(
    group?.name ?? "",
    group ? ["groups", group.id, "name"] : null,
  );
  const [rowElements, setRowElements] = useSavedField<string[]>(
    group?.rowElements ?? [],
    group ? ["groups", group.id, "rowElements"] : null,
  );
  const [elements, setElements] = useSavedField<string[]>(
    group?.elements ?? [],
    group ? ["groups", group.id, "elements"] : null,
  );
