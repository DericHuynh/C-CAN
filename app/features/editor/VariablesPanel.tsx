...
export function VariablesPanel({ project }: VariablesPanelProps) {
  ...
  const [selected, setSelected] = useLiveSelection<Variable>(variables);
  const [deleteTarget, setDeleteTarget] = useState<Variable | null>(null);

  function handleSave(form: VariableFormValues) {
    if (selected === null) return;
    const isNew = selected === "new";
    form = { ...form, id: isNew ? form.id.trim() : selected.id };
    if (
      !form.id ||
      variables.some((item) => item.id === form.id && (isNew || item.id !== selected.id))
    ) {
      toast.error("Use a unique, non-empty ID");
      return;
    }
    const next = isNew
  ...
  function handleDelete() {
    ...
    updateSettings.mutate(
      ...
      {
        onSuccess: () => {
          if (selected !== "new" && selected?.id === target.id) setSelected(null);
        ...
        detail={
          selected === "new" ? (
            <VariableForm
              ...
              onSave={handleSave}
            />
          ) : selected && variables.some((item) => item.id === selected.id) ? (
            <VariableForm
              key={selected.id}
              variable={selected}
              busy={updateSettings.isPending}
...
function VariableForm({ variable, busy = false, onCancel, onSave }: VariableFormProps) {
  const [id, setId] = useSavedField(
    variable?.id ?? "",
    variable ? ["variables", variable.id, "id"] : null,
  );
  const [isTrue, setIsTrue] = useSavedField(
    variable?.isTrue ?? false,
    variable ? ["variables", variable.id, "isTrue"] : null,
  );
