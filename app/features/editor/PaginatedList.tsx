...
export function PaginatedList<T>({
  ...
  const browser = useContentBrowser(allItems);
  ...
  useEffect(() => setPage(0), [pageResetKey, browser.filter]);
  ...
  return (
    <div className={cn("space-y-3", className)}>
      <ContentBrowserControls browser={browser} label={contentLabel} />
      {pager}
      <div style={{ display: "flex", flexDirection: "column", gap }}>
        {pageItems.map((item, index) => (
          <Fragment key={getItemKey(item, browser.results[start + index].index)}>
            {renderItem(item, browser.results[start + index].index)}
          </Fragment>
        ))}
      </div>
      {pager}
