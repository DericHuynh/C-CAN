...
export function RequirementPanel({ project }: RequirementPanelProps) {
  ...
  const [selected, setSelected] = useLiveSelection<GlobalRequirement>(requirements);
...
function RequirementForm({
  ...
  const [value, setValue] = useSavedField(
    item?.name ?? "",
    item ? ["globalRequirements", item.id, "name"] : null,
  );
  const [requireds, setRequireds] = useSavedField<Requireds[]>(
    item?.requireds ?? [],
    item ? ["globalRequirements", item.id, "requireds"] : null,
  );
