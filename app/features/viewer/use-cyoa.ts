...
function storageKey(slot: string): string {
  ...
  const preview =
    typeof window !== "undefined"
      ? new URLSearchParams(window.location.search).has("buildCode")
        ? new URLSearchParams(window.location.search).get("preview")
        : null
      : null;
  return `iccplus-build:${path}${preview ? `:preview:${preview}` : ""}:${slot}`;
}
...
export function useCyoa({ app, initialBuildCode }: UseCyoaOptions): UseCyoaResult {
  const [storageError, setStorageError] = useState(false);
  const [state, setState] = useState<CyoaState>(() =>
    initialBuildCode !== undefined
      ? loadBuildCode(initialBuildCode, app, buildCyoaIndex(app))
      : activateAutomaticChoices(createCyoaState(app), buildCyoaIndex(app), true),
  );
  // A saved-document refresh may introduce IDs absent from the cached document
  // used for the first render. Reapply explicit test states to the fresh index.
  useEffect(() => {
    if (initialBuildCode !== undefined)
      setState(loadBuildCode(initialBuildCode, app, buildCyoaIndex(app)));
  }, [app, initialBuildCode]);
