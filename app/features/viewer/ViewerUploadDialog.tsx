...
export function ViewerUploadDialog({
  ...
  const cropperRef = useRef<ImageCropper | null>(null);
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void import("./image-cropper")
      .then(async ({ createImageCropper }) => {
        if (disposed || !imageRef.current) return;
        const cropper = createImageCropper(imageRef.current, position);
    ...
    >
      ...
      >
            ...
            <>
              ...
              <div className="h-64 overflow-hidden bg-muted">
                <img
                  ...
                  ref={imageRef}
                  src={source}
                  crossOrigin={crop ? "anonymous" : undefined}
        ...
        <DialogFooter>
          ...
          <Button
            disabled={saving || !source || (crop && !ready)}
            onClick={async () => {
              setSaving(true);
              const version = readVersion.current;
              try {
                ...
                if (crop) {
                  ...
                  if (!cropper) return;
                  result = await cropper.export(scale, quality);
