...
import { sortedChoices, sortedRows } from "./project-utils";
 ...
 * canvas; clicking a row or choice selects it and opens its editor in the
 * detail pane, so every change can be previewed immediately. A floating
 * toolbar on the selected item handles add-choice / move / delete.
 */
export function VisualEditor({ project }: VisualEditorProps) {
  ...
  const handleCommitEdit = useCallback(
    (target: VisualEditTarget, value: string) => {
      setEditTarget(null);
      const patch = { [target.field]: value };
      if (target.kind === "row") {
        updateRowMutate(
          { projectId, rowId: target.id, patch },
          {
            onError: (err) =>
      ...
      if (!row) return;
      updateChoiceMutate(
        { projectId, rowId: row.id, choiceId: target.id, patch },
        {
          onError: (err) =>
    ...
    [updateRowMutate, updateChoiceMutate, projectId, rows],
  );
    ...
    (action: VisualEditAction, sel: VisualSelection) => {
      switch (action) {
        ...
        case "move-up":
          ...
          else if (entity?.kind === "choice") handleMoveChoice(entity.row.id, sel.id, -1);
          return;
        case "move-down":
          if (sel.kind === "row") handleMoveRow(sel.id, 1);
          else if (entity?.kind === "choice") handleMoveChoice(entity.row.id, sel.id, 1);
        ...
        case "delete":
          ...
          } else if (entity?.kind === "choice") {
            setDeleteChoiceTarget({ row: entity.row, choice: entity.choice });
          }
          return;
      }
    },
    [entity, handleAddChoice, handleMoveChoice, handleMoveRow, rows],
  );
      ...
      <Card>
        <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            Click a row or choice on the canvas to edit it here, right-click for add-above/below and
            other actions, or double-click a title or description to edit it in place. The preview
            updates as you save — no more dialogs.
          </p>
    ...
    <div className="space-y-3 xl:flex xl:h-full xl:min-h-0 xl:flex-col xl:gap-3 xl:space-y-0">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          ...
          {choiceCount === 1 ? "" : "s"} — click to select, right-click for actions, double-click
