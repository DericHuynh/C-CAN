...
import { useSavedField } from "@/features/editor/use-saved-field";
...
export function InlineTextEditor({
  ...
  html: string;
  livePath?: import("@/features/editor/live-fields").LivePath;
  sourceText?: string;
  ...
  const [draftHtml, setDraftHtml] = useSavedField(sourceText ?? html, livePath);
  const registry = useDraftRegistry();

  // Apply reconciled content only when it differs from the live DOM. Typing
  // retains the browser's selection; unrelated refetches leave the draft alone.
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    if (sourceText !== undefined) {
      if (el.textContent !== draftHtml) el.textContent = draftHtml;
    } else if (el.innerHTML !== draftHtml) el.innerHTML = draftHtml;
  }, [draftHtml, sourceText]);
  ...
  async function commit() {
    ...
    try {
      ...
      await onCommit(ref.current?.textContent ?? "");
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Could not save text. Your draft is still open.",
      );
    } finally {
