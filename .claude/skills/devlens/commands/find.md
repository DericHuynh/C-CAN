...
A compact list of matching nodes: **name** — `filePath:lines` — type. The compact refs already include a short `summary` and `severity` — surface the summary when present so the user sees what each match _does_ (no extra call, no file reads). No source. If there are many, group by file/folder and surface the most central first. Then offer the natural next step: `/devlens summary <id>` to understand one, or `/devlens impact <id>` to see what it touches.
