...
- **Empty/thin result caveat:** an empty or surprisingly small blast radius means "no callers _in the graph_," not "safe to change." The real caller may be an un-extracted call site (a `fetch`/`axios` inside an object-literal method or config registry, or a URL used as `src`/`href`). Say "no graph-visible callers" and confirm with a targeted `Grep` before asserting safety.
