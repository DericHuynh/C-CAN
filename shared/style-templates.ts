/**
 * Preset styling templates, ported verbatim from the original ICCPlus
 * creator (src/lib/creator/Features/AppTemplates.svelte). Applying a
 * template merges it into app.styling (same as the original changeStyling()).
 */
import type { Styling } from "./types.js";

export const STYLE_TEMPLATE_NAMES = [
  "Fall",
  "Book",
  "Bone",
  "Dark",
...
/** Apply template index into an existing styling object (returns a new object). */
export function applyStyleTemplate(
  styling: Record<string, unknown> | undefined,
  index: number,
): Styling {
  const template = STYLE_TEMPLATES[index] ?? STYLE_TEMPLATES[0];
  return { ...(styling ?? {}), ...template } as Styling;
}

export const STYLE_TEMPLATES: Array<Record<string, unknown>> = 
[{
        rowTitle: 'Courier',
        rowText: 'Georgia',
        objectTitle: 'Courier',
